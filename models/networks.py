...
from models.mimo.MIMOUNet import MIMOUNet, MIMOUNetPlus
...
def define_D_exp(input_nc, ndf, netD, n_layers_D=3, norm="batch", init_type="normal", init_gain=0.02, gpu_ids=[]):
    """Create a discriminator
    ...
    if netD == "basic":  # default PatchGAN classifier
        net = NLayerDiscriminator1(input_nc, ndf, n_layers=3, norm_layer=norm_layer)
    elif netD == "n_layers":  # more options
        net = NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer)
    elif netD == "pixel":  # classify if each pixel is real or fake
        net = PixelDiscriminator(input_nc, ndf, norm_layer=norm_layer)
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(
        ...
        downrelu = nn.SiLU()  # Replaced  nn.LeakyReLU(0.2, True)
        ...
        if outermost:
            ...
            upconv = [nn.Conv2d(inner_nc * 2, outer_nc, kernel_size=5, padding=2), nn.SiLU()]
            ...
            upsample = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True)
            upconv = [nn.Conv2d(inner_nc, outer_nc, kernel_size=5, padding=2), nn.SiLU()]
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        kw = 4
        padw = 1
        sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.SiLU()]
        nf_mult = 1
        nf_mult_prev = 1
    ...
    def forward_D(self, x):
        """Get Attention forward."""
        feat = 0
        for ind, layer in enumerate(self.model):
            print(layer)
            x = layer(x)
            if ind in [4]:
                feat = np.absolute(x.cpu().detach()).mean(1)
        return feat
    ...
    def forward(self, x):
        ...
        outputs = []
        for each in x:
            outputs.append(self.model(each))
        return outputs
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.net = [
            ...
            nn.SiLU(),  # Replaced LeakyReLU(0.2, True)
